...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .priority-p2 { border-left-color: #2ed573; }
    </style>
</head>
<body>
    <div class="container">
        ...
        function displayResult(data) {
            ...
            let html = `
                <div class="result-card">
                    <h2 style="margin-bottom: 20px;">📊 诊断结果</h2>
                    <div class="score-grid">
                        ...
                        <div class="score-item">
                            <div class="score-value">${scores.visibility_score}%</div>
                            <div class="score-label">AI可见度</div>
                        </div>
                        <div class="score-item">
                            <div class="score-value">${scores.mention_rate}%</div>
                            <div class="score-label">品牌提及率</div>
                        </div>
                        <div class="score-item">
                            <div class="score-value">${scores.accuracy_score}%</div>
                            <div class="score-label">信息准确度</div>
                        </div>
                    </div>
                </div>
            `;
            ...
            if (suggestions.length > 0) {
                html += `<div class="result-card"><h3 style="margin-bottom: 16px;">💡 优化建议</h3>`;
                suggestions.forEach(s => {
                    const priority = s.priority || 'P1';
                    html += `
                        <div class="suggestion-item priority-${priority.toLowerCase()}">
                            <strong>[${priority}] ${s.title}</strong>
                            <p style="margin-top: 8px; color: #666;">${s.expected}</p>
                        </div>
                    `;
                });
                html += `</div>`;
...
async def diagnose(
    ...
    for platform in platforms:
        system_prompt = f"""分析品牌'{brand_name}'在{platform}平台的表现。返回JSON格式：
{{"brand_mentioned": true/false, "mention_position": 数字或null, "mention_count": 数字, "sentiment": "positive/neutral/negative"}}"""
        
        messages = [
            {"role": "system", "content": system_prompt},
            {"role": "user", "content": f"分析{industry}品牌{brand_name}"}
        ]
    ...
    # 生成建议
    suggestions = [
        {"priority": "P0", "title": "官网添加FAQ页面", "expected": "AI提及率提升30%+"},
        {"priority": "P0", "title": "优化品牌基础信息", "expected": "信息准确度提升20%"},
        {"priority": "P1", "title": "内容矩阵建设", "expected": "提升品牌权威性"},
        {"priority": "P2", "title": "社交媒体优化", "expected": "增加品牌曝光"}
    ]
